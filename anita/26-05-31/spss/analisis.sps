...
********************************************************************************
* BLOQUE 9. TEST DE FRIEDMAN POR NIVEL EDUCATIVO
...
*   Implementación: TEMPORARY + SELECT IF (v2.6).
*   Cambio desde v2.5: SPLIT FILE generaba error 4757 (límite de 8
*   variables de archivo segmentado, agotado por las 15+ variables
*   string del dataset) y producía un solo Friedman combinado.
*   TEMPORARY + SELECT IF es el patrón canónico de SPSS para análisis
*   por subgrupo: filtra los casos de forma temporal sin tocar el
*   archivo de trabajo, y la cláusula TEMPORARY se revierte
*   automáticamente al cierre del bloque de comandos.
********************************************************************************.

* Grupo 1: Secundaria.
TEMPORARY.
SELECT IF (nivel_agrupado = 'Secundaria').
NPAR TESTS
  /FRIEDMAN=
    p01_has_escuchado_educacion_financiera
    p02_conoces_significado_educacion_financiera
...
TEMPORARY.
SELECT IF (nivel_agrupado = 'Bachillerato').
NPAR TESTS
  /FRIEDMAN=
    p01_has_escuchado_educacion_financiera
    p02_conoces_significado_educacion_financiera
...
* Restaurar el dataset completo (TEMPORARY ya lo hace al cerrar el
* NPAR TESTS, pero la línea explícita es defensiva).
FILTER OFF.
EXECUTE.
