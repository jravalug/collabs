...
********************************************************************************
* BLOQUE 15. PYTHON - CONTEO DE GRAFICOS EN EL VISOR
*   Solo cuenta los graficos generados (no exporta). El usuario exporta
*   manualmente desde el Visor: clic derecho sobre cada grafico > Exportar > PNG.
*   Los 4 graficos enfocados (P18, P04, P10, P06) estan identificados por titulo
*   en BLOQUE 13; tambien aparecen otros graficos automaticos de Frequencies/
*   Crosstabs que el usuario puede ignorar.
********************************************************************************.
...
try:
    ...
    if outputDocs.Size() == 0:
        ...
        for i in range(outputItems.Size()):
            item = outputItems.GetItemAt(i)
            if item.GetType() == 3:
                count += 1
    SpssClient.StopClient()
except Exception as e:
    print('Error al contar graficos: ' + str(e))

print('--- Graficos detectados en el Visor: %d ---' % count)
print('--- Exportacion automatica DESHABILITADA ---')
print('--- Exportar manualmente: clic derecho sobre cada grafico > Exportar > PNG ---')
END PROGRAM.
...
try:
    ...
    try:
        ...
        for i in range(varCount):
            existing_vars.append(spss.GetVariableName(i))
    except Exception as e:
        errores.append('No se puede acceder al dataset activo: ' + str(e))
        existing_vars = []
    ...
    if outputDocs.Size() == 0:
        ...
        for i in range(outputItems.Size()):
            item = outputItems.GetItemAt(i)
            tipo = item.GetType()
            if tipo == SpssClient.OutputItemType.PIVOT:
                tablas += 1
            elif tipo == SpssClient.OutputItemType.CHART:
                graficos += 1
            elif tipo == SpssClient.OutputItemType.WARNING:
                errores_output += 1
